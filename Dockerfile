...
# `npm run build` runs some django commmands to get its config.
# But django cannot load its config because the static/manifest.json doesn't 
# exists.
# File generated by the `npm run build` command…
# Break the loop by creating an empty manifest
